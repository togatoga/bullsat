...
### リテラルの設計
リテラルは`正`のリテラル(\\(x_1\\))と`負`のリテラル((\lnot x_1\\))が存在します。少しトリッキーなのですが、正のリテラルは`偶数`、負のリテラルは`奇数`として表現します。

- 正のリテラル: \\(x_0\\)は`0`、\\(x_1\\)は`2`、\\(x_2\\)は`4`
- 負のリテラル: \\(\lnot x_0\\)は`1`、\\(\lnot x_1\\)は`3`、\\(\lnot x_2\\)は`5`

正/負のリテラルから命題変数を取得するには現在の値から1ビット右にシフト(1/2)するだけです。
...
以下の関数は`Variable`や`Lit`をindexとして`std::vector`のデータ構造が必要となるため定義した。
